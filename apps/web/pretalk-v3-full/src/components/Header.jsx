...
import { NAV_FEATURES, CASE_STUDIES } from "../data/index";

function Logo() {
  return (
    <Link to="/" className="flex items-center gap-2.5 no-underline flex-shrink-0">
      <div className="w-[30px] h-[30px] bg-black rounded-[7px] flex items-center justify-center">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          <path d="M3 8h10M8 3l5 5-5 5" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      </div>
      <span className="text-[17px] font-bold text-black tracking-[-0.4px]">pretalk</span>
    </Link>
  );
}
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-3 gap-2.5">
          {NAV_FEATURES.map((f, i) => (
            <Link key={i} to={`/fonctionnalites#${f.label.toLowerCase().replace(/\s+/g,"-")}`} onClick={onClose}
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
          ...
          <Link to="/fonctionnalites" onClick={onClose}
    ...
    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-[820px] glass rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.12)] animate-slideDown overflow-hidden">
      <div className="p-6">
        <div className="grid grid-cols-[1.6fr_1fr] gap-5">
          {/* Featured */}
          {featured && (
            <Link to={`/blog/${featured.slug}`} onClick={onClose}
          ...
          <div className="flex flex-col gap-1">
            ...
            {rest.map((cs, i) => (
              <Link key={i} to={`/blog/${cs.slug}`} onClick={onClose}
            ...
            <Link to="/blog" onClick={onClose} className="mt-3 text-[12px] font-semibold text-black hover:text-green-700 flex items-center gap-1 transition-colors px-2.5">
    ...
    <header ref={ref} className={`fixed top-0 left-0 right-0 z-50 h-[60px] flex items-center border-b border-black/[0.06] transition-all duration-300 ${scrolled?"glass shadow-[0_2px_20px_rgba(0,0,0,0.07)]":"bg-white/90 backdrop-blur-xl"}`}>
      <div className="max-w-[1160px] mx-auto px-8 w-full flex items-center justify-between">
          ...
          {nav.map((item) => (
            <div key={item.label} className="relative">
              {item.key ? (
                ...
                <Link to={item.href} className="text-[13.5px] font-medium text-gray-600 hover:text-black hover:bg-gray-100 px-3 py-1.5 rounded-lg transition-all duration-150 block">
                  {item.label}
                </Link>
              )}
        ...
        <div className="flex items-center gap-2">
          <Link to="/contact" className="text-[13.5px] font-medium text-gray-700 px-4 py-1.5 rounded-lg hover:bg-gray-100 transition-colors">
            Se connecter
          </Link>
          <Link to="/tarifs" className="inline-flex items-center gap-1.5 text-[13.5px] font-semibold text-white bg-black px-4 py-2 rounded-xl hover:bg-gray-800 hover:-translate-y-px hover:shadow-md transition-all duration-200">
            Essai gratuit
            <ArrowRight size={13}/>
          </Link>
        </div>
