...
import { Btn, Reveal, Badge, GlassCard, ArrowRight, ClockIcon } from "../components/ui";
...
export default function CaseStudy() {
  ...
  if (!cs) return (
    <div className="pt-32 pb-20 px-8 text-center">
      ...
      <Link to="/blog" className="text-green-600 underline">Retour au blog</Link>
    </div>
  );
        ...
        <div className="max-w-[900px] mx-auto px-8 pt-12 pb-10 relative z-10">
          <Reveal>
            <div className="flex items-center gap-3 mb-5">
              <Link to="/blog" className="text-[12px] text-white/40 hover:text-white/70 transition-colors">← Blog</Link>
        ...
        <div className="max-w-[900px] mx-auto">
          ...
          <Reveal>
            <div className="my-16 bg-black rounded-3xl p-8 relative overflow-hidden">
              ...
              <Btn to="/tarifs" variant="white" size="md">Essayer gratuitement <ArrowRight color="black"/></Btn>
            </div>
          </Reveal>
        </div>
      </section>
            ...
            <div className="grid grid-cols-2 gap-5">
              {related.map((r, i) => (
                <Reveal key={i} delay={i*80}>
                  <Link to={`/blog/${r.slug}`} className="group block no-underline bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-md hover:-translate-y-1 transition-all duration-200">
