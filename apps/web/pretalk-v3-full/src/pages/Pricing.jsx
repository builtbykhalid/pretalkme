...
import { Btn, Reveal, SL, ST, Badge, GlassCard, Check, ArrowRight } from "../components/ui";
          ...
          <div className="grid grid-cols-4 gap-5 mb-20">
            {PLANS.map((p, i) => (
              <Reveal key={i} delay={i*70}>
                <div className={`relative rounded-3xl p-7 border-[1.5px] transition-all hover:shadow-lg ${p.featured?"border-black bg-white shadow-md":"border-gray-200 bg-white hover:-translate-y-1"}`}>
                  {p.featured && (
                  ...
                  <Btn to="/contact" variant={p.featured?"black":"outline"} size="sm" className="w-full mb-5">{p.cta}</Btn>
                  <ul className="flex flex-col gap-2">
                    {p.features.map((f, j) => (
                      <li key={j} className="flex items-start gap-2 text-[12px] text-gray-600">
                        <svg className="flex-shrink-0 mt-0.5 text-green-500" width="13" height="13" viewBox="0 0 14 14" fill="none">
      ...
      <section className="py-20 px-8 bg-gray-50 text-center">
        ...
        <Reveal delay={160}>
          <div className="flex items-center justify-center gap-3">
            <Btn to="/faq" variant="outline">Voir la FAQ <ArrowRight/></Btn>
            <Btn to="/contact" variant="black">Nous contacter <ArrowRight/></Btn>
          </div>
        </Reveal>
      </section>
    </>
