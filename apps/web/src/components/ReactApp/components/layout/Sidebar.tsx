...
export default function Sidebar({ mobile = false, isOpen = true, onToggle }: { mobile?: boolean; isOpen?: boolean; onToggle?: () => void }) {
  ...
  const navItems = [
    ...
    ...(isFreelance ? [{ id: 'service-cases', label: 'Dossiers de Service', icon: Briefcase, path: '/service-cases' }] : []),
  ...
  return (
    <>
      {/* 40px app-switcher rail — always visible on desktop */}
      {!mobile && <AppSwitcher currentApp="whatsapp" />}

      <div
        className={mobile
          ? 'relative h-full w-64 bg-white border-r border-[#D1D7DB] flex flex-col z-30 md:hidden'
          : `fixed top-0 h-full bg-white border-r border-[#D1D7DB] flex-col z-40 hidden md:flex transition-all duration-300 ${isOpen ? 'w-64' : 'w-20'} left-10`
        }
      >
        {/* Branding */}
        <div className={`flex items-center justify-between ${isOpen ? 'px-5 py-4' : 'px-0 py-4 justify-center'}`}>
          {isOpen ? (
            <div className="flex flex-col leading-tight">
              <span className="text-[11px] font-medium text-[#8696A0] uppercase tracking-widest">Pretalk</span>
              <span className="text-[15px] font-bold text-[#111B21]">WhatsApp</span>
            </div>
          ) : null}
          {isOpen && (
