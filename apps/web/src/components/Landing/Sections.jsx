...
const STEPS = [
  ...
  {
    icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><rect x="4" y="5" width="16" height="12" rx="2" stroke="#0A0A0A" strokeWidth="1.7"/><path d="M8 10h8M8 13h5" stroke="#0A0A0A" strokeWidth="1.4" strokeLinecap="round"/><circle cx="19" cy="5" r="3" fill="#48D951"/></svg>,
    num: "Étape 02", title: "L'IA qualifie", desc: "Formulaire IA + scoring automatique en temps réel.",
  },
  {
    icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2" stroke="#0A0A0A" strokeWidth="1.7"/><rect x="9" y="3" width="6" height="4" rx="1" stroke="#0A0A0A" strokeWidth="1.5"/><path d="M9 12l2 2 4-4" stroke="#48D951" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/></svg>,
    ...
    <section id="pipeline" className="py-24 px-8 bg-white">
      <div className="max-w-[1160px] mx-auto">
        ...
        <div className="relative grid grid-cols-5 gap-0">
          {/* Line */}
          <div className="absolute top-[31px] left-[10%] right-[10%] h-px bg-gradient-to-r from-gray-200 via-primary-500 to-gray-200" />

          {STEPS.map((s, i) => (
            <Reveal key={i} delay={i * 80}>
              <div className="group flex flex-col items-center text-center px-3 relative z-10">
                <div className="w-[62px] h-[62px] rounded-full bg-white border-[1.5px] border-gray-200 flex items-center justify-center mb-[18px] shadow-sm transition-all duration-250 group-hover:border-primary-500 group-hover:shadow-[0_0_0_5px_var(--color-primary-50)] group-hover:scale-110">
                  {s.icon}
                </div>
      ...
      <div className="max-w-[1160px] mx-auto">
        ...
        <Reveal delay={80}>
          <div className="flex items-center justify-center gap-3 mb-14 text-[13.5px] text-gray-500">
            <span>Mensuel</span>
            <div className="w-[42px] h-[23px] bg-primary-500 rounded-full relative cursor-pointer shadow-lg shadow-primary-500/20">
          ...
          {PLANS.map((plan, i) => (
            <Reveal key={i} delay={i * 80}>
              <div className={`relative rounded-3xl p-7 border transition-all duration-200 hover:shadow-md ${plan.featured ? "border-black border-2" : "border-gray-200"}`}>
                ...
                {plan.cta.outline ? (
                  <BtnOutline href="#" onClick={(e) => { e.preventDefault(); onCTA?.(); }} className="w-full justify-center mb-5 py-[11px]">{plan.cta.label}</BtnOutline>
                ) : (
                  <BtnPrimary href="#" onClick={(e) => { e.preventDefault(); onCTA?.(); }} className="w-full justify-center mb-5 py-[11px] rounded-xl text-[13.5px]">{plan.cta.label}</BtnPrimary>
                )}
...
export function CTA({ onCTA }) {
  return (
    <section className="relative py-[120px] px-8 bg-black text-center overflow-hidden">
      <div className="absolute top-[-180px] left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-[radial-gradient(circle,rgba(72,217,81,0.13)_0%,transparent_65%)] pointer-events-none" />
      <div className="max-w-[1160px] mx-auto relative z-10">
        ...
        <Reveal delay={240}>
          <div className="flex items-center justify-center gap-3 flex-wrap">
            <BtnHero href="#" white onClick={(e) => { e.preventDefault(); onCTA?.(); }}>
              Démarrer gratuitement — c'est gratuit <ArrowIcon color="black" />
            </BtnHero>
            <BtnOutline href="#" dark onClick={(e) => { e.preventDefault(); onCTA?.(); }}>
