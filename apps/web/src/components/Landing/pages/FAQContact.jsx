...
import { Btn, Reveal, SL, ST, Badge, GlassCard, ArrowRight, ChevronDown } from "../ui";
import { FAQS } from "../../../data/landing/index";
...
function FAQSection() {
  ...
  const cats = ["Tout", ...availableCats];
  const [activeCat, setActiveCat] = useState("Tout");

  const filtered = activeCat === "Tout" ? FAQS : FAQS.filter(f=>f.cat===activeCat);
        ...
        <div className="text-center space-y-6">
          ...
          <Reveal delay={80}>
            <h1 className="text-4xl md:text-6xl font-black text-dark tracking-tighter leading-tight">
              Toutes les réponses.<br/><span className="text-primary-500">Toute l'aide.</span>
            </h1>
          </Reveal>
          <Reveal delay={160}>
            <p className="text-neutral-500 font-medium max-w-2xl mx-auto text-lg leading-relaxed">
              Trouvez la réponse dans la FAQ ou écrivez-nous directement. Nous répondons en moins de 4h.
            </p>
          </Reveal>
        </div>
      </section>
