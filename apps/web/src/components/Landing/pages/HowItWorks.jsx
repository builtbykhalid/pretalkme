import { Btn, Reveal, SL, ST, Badge, GlassCard, ArrowRight, ImgPh, CheckCircle } from "../ui";
...
export default function HowItWorks() {
  return (
    <>
        ...
        <div className="text-center space-y-6">
          ...
          <Reveal delay={80}>
            <h1 className="text-4xl md:text-6xl font-black text-dark tracking-tighter leading-tight">
              De l'onboarding à la <span className="text-primary-500">fidélisation.</span>
            </h1>
          </Reveal>
          <Reveal delay={160}>
            <p className="text-neutral-500 font-medium max-w-2xl mx-auto text-lg leading-relaxed">
              Pretalk implémente les 12 frameworks des meilleurs cabinets de conseil (McKinsey, Bain, HBS) dans un système automatisé simple à activer.
            </p>
          </Reveal>
          <Reveal delay={240}>
            <div className="flex items-center justify-center gap-3 pt-4">
              <Btn href="/tarifs" variant="black" size="lg">Commencer maintenant <ArrowRight/></Btn>
              <Btn href="/contact" variant="outline" size="lg">Voir la démo live</Btn>
            </div>
          </Reveal>
        </div>
      </section>
      ...
      <section className="py-24 px-6 md:px-8 bg-gray-50/50">
        <div className="max-w-7xl mx-auto">
          <Reveal>
            <div className="text-center mb-16 space-y-4">
              <h2 className="text-3xl md:text-4xl font-black text-dark tracking-tight">
                En résumé : <span className="text-primary-500">60 jours</span> pour transformer votre activité.
              </h2>
              <p className="text-neutral-500 font-medium max-w-2xl mx-auto">
                Un accompagnement par phase pour passer d'un process manuel à une machine de vente automatisée.
              </p>
            </div>
          </Reveal>
          ...
          <div className="relative">
            {/* Desktop Line */}
            <div className="hidden md:block absolute top-[44px] left-[5%] right-[5%] h-0.5 bg-neutral-200" />
            
            <div className="grid grid-cols-1 md:grid-cols-5 gap-8 md:gap-4 relative z-10">
              {[
                "Onboarding & setup (10 min)",
                "Qualification automatique active",
                "Premier audit IA pré-call",
                "Premier devis & contrat généré",
                "Premier retainer proposé"
              ].map((t, i) => (
                <Reveal key={i} delay={i * 100}>
                  <div className="flex flex-col items-center text-center group">
                    {/* Circle */}
                    <div className="w-20 h-20 rounded-full bg-white border-2 border-neutral-100 flex items-center justify-center mb-6 shadow-sm group-hover:border-primary-500 group-hover:shadow-lg transition-all duration-300 relative z-10">
                      <div className="w-14 h-14 rounded-full bg-neutral-50 flex items-center justify-center group-hover:bg-primary-50 transition-colors">
                        <span className="text-xl font-black text-neutral-400 group-hover:text-primary-600">
                          {i + 1}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary-600 mb-1">Phase {i}</p>
                      <h4 className="text-[14px] font-bold text-dark leading-tight px-2">
                        {t}
                      </h4>
                    </div>
                  </div>
