...
export class ContractsService {
  ...
  private async resolveEngagement(tenantId: string, dto: CreateContractDto): Promise<string | null> {
  ...
  async create(tenantId: string, userId: string, dto: CreateContractDto) {
    ...
    const ref = `CONTRAT-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 90000) + 10000)}`;
    ...
    if (dto.conversation_id) {
      await supabase.from('messages').insert({
        ...
        direction: 'outbound',
        type: 'note',
        content: `📄 Contrat créé : ${dto.title}${dto.amount ? ` — ${dto.amount} ${dto.currency || 'MAD'}` : ''} (${ref})`,
      });
    }
  ...
  async findAll(tenantId: string, sourceApp?: string) {
    ...
    if (sourceApp) query = query.eq('created_by_app', sourceApp);
  ...
  private buildContractPdfBuffer(input: {
    title: string;
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.fontSize(24).text('Contrat', { align: 'left' });
      doc.moveDown(0.5);
      ...
      doc.fillColor('#111').fontSize(13).text('Objet du contrat');
      ...
      doc.fontSize(10).fillColor('#666').text('Document genere automatiquement par Pretalk.', { align: 'left' });
  ...
  async generatePdf(tenantId: string, id: string) {
    ...
    const pdfBuffer = await this.buildContractPdfBuffer({
      title: docRow.title,
    ...
    const storagePath = `contracts/${tenantId}/${id}.pdf`;
    const uploadRes = await (supabase as any).storage.from(this.documentsBucket).upload(storagePath, pdfBuffer, {
  ...
  async sendViaWhatsApp(tenantId: string, id: string) {
    ...
    const message = documentUrl
      ? `📄 Votre contrat est disponible : ${documentUrl}`
      : `📄 Votre contrat "${doc.title || doc.reference || 'Contrat'}" est pret.`;
